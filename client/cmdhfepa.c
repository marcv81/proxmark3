...
// Perform the initial part of the PACE protocol and collect the random nonce
...
int CmdHFEPAPACEReplay(const char *Cmd)
{
	...
	for (int i = 0; i < sizeof(apdu_lengths); i++) {
		...
		while (Cmd[skip] != ' ' && Cmd[skip] != '\0') {
			// convert 
			scan_return = sscanf(Cmd + skip, "%2hhX%n",
	...
	resp = WaitForResponse(CMD_ACK);
	if (resp->arg[0] != 0) {
		PrintAndLog("PACE replay failed in step %u!", (uint32_t)resp->arg[0]);
...
static const command_t CommandTable[] = 
{
  ...
  {"preplay", CmdHFEPAPACEReplay,        0,
   "<mse> <map> <pka> <ma> Perform PACE protocol by replaying given APDUs"},
  {NULL, NULL, 0, NULL}
};
